...
func TestPanelsUseRoundedIntegratedTitlesAndResponsiveBranding(t *testing.T) {
	t.Setenv("TERMCOURSE_COLOR_MODE", "truecolor")
	style := NewStyle(testTheme(), &bytes.Buffer{})
	compact := style.AppHeader("Latest Topics", "community.example", "9.8.7", []string{"arrows: move | q: quit", "Logged in: member"}, 52, 24)
	...
	wide := stripANSI(strings.Join(style.AppHeader("Latest", "community.example", "9.8.7", []string{"controls", "status"}, 90, 32), "\n"))
	if !strings.Contains(wide, "▀█▀ █▀▀ █▀█ █▀▄▀█") || !strings.Contains(wide, "◉ DISCOURSE TERMINAL · VERSION 9.8.7") || strings.Contains(wide, "· TEST") || !strings.Contains(wide, "● ONLINE") {
